...
namespace SkyRoof
{
  public partial class MainForm : Form
  {
    ...
    private void MainForm_FormClosing(object sender, EventArgs e)
    {
      ...
      ctx.Sdr?.Dispose();
      ctx.Slicer?.Dispose();
      if (ctx.SstvDecoder != null)
      {
        ctx.SstvDecoder.ImageDecoded -= SstvDecoder_ImageDecoded;
        ctx.SstvDecoder.Dispose();
        ctx.SstvDecoder = null;
      }
    ...
    private void ConfigureSlicer()
    {
      ...
      ctx.SstvDecoder = new SstvDecoderService(Slicer.OUTPUT_SAMPLING_RATE);
      ctx.SstvDecoder.ImageDecoded += SstvDecoder_ImageDecoded;
    }
    ...
    private void Slicer_AudioDataAvailable(object? sender, DataEventArgs<float> e)
    {
      ...
      var sampleCount = e.Count > 0 ? e.Count : e.Data.Length;
      if (ctx.SstvDecoder != null)
        ctx.SstvDecoder.Feed(e.Data, sampleCount);
    ...
    private void SstvDecoderMNU_Click(object sender, EventArgs e)
    {
      if (ctx.SstvPanel == null)
        ShowFloatingPanel(new SstvPanel(ctx));
      else
        ctx.SstvPanel.Close();
    }

    private void SstvDecoder_ImageDecoded(object? sender, SstvImageEventArgs e)
    {
      if (ctx.SstvPanel != null)
      {
        ctx.SstvPanel.UpdateImage(e.Image, e.IsComplete);
      }
      else
      {
        e.Image.Dispose();
