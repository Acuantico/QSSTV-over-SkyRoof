  ...
  internal static class Utils
  {
    ...
    internal static string GetVersionNumber()
    {
      try
      {
        ...
        string overridePath = Path.Combine(baseDir, "VERSION_OVERRIDE");
        if (File.Exists(overridePath))
        {
          string content = File.ReadAllText(overridePath).Trim();
      ...
      {
        Debug.WriteLine($"Failed to read version override: {ex}");
      }
